...
function renderMessage(message, isCurrentUser) {
    ...
    messageElement.appendChild(messageBubble); // Append the bubble to the wrapper
...
async function displayChatView(chatInfo) {
    ...
    unsubscribeMessages = onSnapshot(q, async (snapshot) => {
        // Clear existing messages to re-render, or handle individual changes
        // For simplicity, re-rendering all messages on any change. For large chats,
        // you'd optimize this to only add/remove/update changed messages.
        chatMessagesContainer.innerHTML = '';
        for (const docChange of snapshot.docChanges()) {
            if (docChange.type === 'added') {
                const messageData = docChange.doc.data();
                // Fetch sender's display name
                const senderDocRef = doc(db, `artifacts/${appId}/users`, messageData.senderId);
                let senderDocSnap;
                try {
                    senderDocSnap = await getDoc(senderDocRef);
                } catch (fetchError) {
                    console.error("Error fetching sender document:", fetchError);
                    senderDocSnap = null; // Ensure it's explicitly null on error
                }
                
                const senderName = (senderDocSnap && senderDocSnap.exists()) ? (senderDocSnap.data().username || senderDocSnap.data().email) : 'Desconocido';

                renderMessage({ ...messageData, senderName }, messageData.senderId === currentUserId);
            }
        }
    }, (error) => {
    ...
    groups.forEach(group => {
        ...
        groupElement.addEventListener('click', () => {
            displayChatView({
                ...
                type: 'group',
                name: group.name,
                pic: "https://placehold.co/50x50/E2E8F0/4A5568?text=Group", // Placeholder for group pic
                status: `${group.members.length} miembros`
            });
        });

        const groupPic = document.createElement('img');
        groupPic.src = "https://placehold.co/50x50/E2E8F0/4A5568?text=Group";
...
function listenForFriendRequests() {
    ...
    onSnapshot(q, async (snapshot) => {
        ...
        notifications = notifications.filter(n => n.type !== 'friendRequest'); 
        ...
        for (const docChange of snapshot.docChanges()) {
            if (docChange.type === 'added' || docChange.type === 'modified') {
                ...
                // If a request is removed (e.g., accepted/denied by another instance), remove its notification
                notifications = notifications.filter(n => !(n.type === 'friendRequest' && n.data && n.data.requestId === docChange.doc.id));
            }
        }
        updateNotificationBadge();
        ...
        addNotification('Error de Solicitudes', 'Hubo un problema al cargar las solicitudes de amistad.', 'error');
    });
}
